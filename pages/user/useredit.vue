	...
	<view>
		...
		<form>
			...
			<view class="cu-form-group margin-top">
				...
				<input placeholder="所在部门" name="input" v-model="myFormData.orgCodeTxt" disabled></input>
			</view>
			<view class="cu-form-group">
...
export default {
  ...
  methods: {
    onSubmit() {
      ...
      this.$tip.loading();
      this.$http
        .put("/sys/user/appEdit", this.myFormData)
        .then((res) => {
          console.log(res);
          this.$tip.loaded();
          if (res.data.success) {
    ...
    ChooseImage() {
      ...
      uni.chooseImage({
        ...
        success: (res) => {
          that.$http
            .upload(that.$config.apiUrl + that.uploadUrl, {
